...
export const passportCall = (strategy) => {
    return async (req, res, next) => {
        console.log(`Ejecutando middleware passportCall para estrategia: ${strategy}`);

        passport.authenticate(strategy, (err, user, info) => {
            if (err) return next(err);

            if (!user) {
                return res.status(401).send({ status: "error", message: info.messages ? info.messages : info.toString() });
            }


            console.log("Usuario autenticado con exito: ", user);
            req.user = user;
            next();
        })(req, res, next);
    }
...
export const authorization = (role) => {
    return async (req, res, next) => {
        console.log(`Ejecutando middleware authorization para rol: ${role}`);
        if (!req.user) {
        ...
        console.log("Usuario autorizado con exito: ", req.user);
        next();
    }
}
